...
if __name__ == "__main__":
    ...
    if not image_file_paths:
        ...
        # Getting the base64 strings for selected images
        base64_images = encode_images(image_file_paths)
    ...
    # Process the answer to properly format the output
    pattern = r"```json\s*(\{.*?\})\s*```"
    ...
    if match:
        ...
        try:
            ...
            # Write the original answer if JSON parsing fails
            with open('output.txt', 'w') as f:
                f.write(f"Response: {answer}\n")
    else:
        # No JSON code block found, write the answer as is
        with open('output.txt', 'w') as f:
            f.write(f"Response: {answer}\n")
